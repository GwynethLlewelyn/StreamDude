...
(function (root, factory) {
	...
	var BootstrapDialogModal = function (element, options) {
		if (
			bootstrap &&
			bootstrap.Modal &&
			/^5\.1\./.test(bootstrap.Modal.VERSION)
		) {
			return new Modal(element, options);
		} else if (/4\.1\.\d+/.test($.fn.modal.Constructor.VERSION)) {
			//FIXME for BootstrapV4
			return new Modal(element, options);
		} else {
	...
	BootstrapDialog.METHODS_TO_OVERRIDE["v3.1"] = {
		...
		updateZIndex: function () {
			if (this.isOpened()) {
				...
				$backdrop.css(
					"z-index",
					zIndexBackdrop + (dialogCount - 1) * 20
				);
			}
	...
	BootstrapDialog.METHODS_TO_OVERRIDE["v3.2"] = {
		updateZIndex:
			BootstrapDialog.METHODS_TO_OVERRIDE["v3.1"]["updateZIndex"],
	...
	BootstrapDialog.prototype = {
		...
		getButtonSize: function () {
			if (
				typeof BootstrapDialog.BUTTON_SIZES[this.getSize()] !==
				"undefined"
			) {
		...
		enhanceButton: function ($button) {
			...
			$button.toggleSpin = function (spin) {
				...
				if (spin) {
					...
					$button.prepend(
						dialog
							.createButtonIcon(dialog.getSpinicon())
							.addClass("icon-spin")
					);
				} else {
					$icon.show();
		...
		handleModalEvents: function () {
			...
			this.getModal().on("keyup", { dialog: this }, function (event) {
				var dialog = event.data.dialog;
				if (
					typeof dialog.registeredButtonHotkeys[event.which] !==
					"undefined"
				) {
					var $button = $(
						dialog.registeredButtonHotkeys[event.which]
					);
		...
		isModalEvent: function (event) {
			return (
				typeof event.namespace !== "undefined" &&
		...
		realize: function () {
			...
			this.getModal().data(
				"bs.modal",
				new BootstrapDialogModal(
					this.getModalForBootstrapDialogModal(),
					{
						//FIXME for BootstrapV4
						backdrop:
							this.isClosable() && this.canCloseByBackdrop()
								? true
								: "static",
						keyboard: false,
						show: false,
	...
	BootstrapDialog.confirm = function () {
		...
		if (
			confirmOptions.btnsOrder === BootstrapDialog.BUTTONS_ORDER_OK_CANCEL
		) {
			buttons.reverse();
		}
